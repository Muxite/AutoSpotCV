...
modelFile = r"face recognition\res10_300x300_ssd_iter_140000_fp16.caffemodel"
configFile = r"face recognition\deploy.prototxt"
...
def preprocess(location):
    ...
    for person in people:
        ...
        print(f"Training recognition on {person}")
...
def train(train_location=r'face recognition\Faces', save_location=r'D:\face_trained.yml'):
...
def test(test_location=r'face recognition\Tests', read_location=r'D:\face_trained.yml'):
    net = cv.dnn.readNetFromCaffe(configFile, modelFile)
    net.setPreferableBackend(cv.dnn.DNN_BACKEND_CUDA)
    net.setPreferableTarget(cv.dnn.DNN_TARGET_CUDA)
    features, labels, people = preprocess(test_location)
    face_recognizer = cv.face.LBPHFaceRecognizer_create()
